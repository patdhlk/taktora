...
.. req:: CAN backend emits WireType impls via the wire runtime
   ...
   :status: implemented
   :satisfies: FEAT_0115
   :links: BB_0115, TEST_0923, TEST_0924, TEST_0925
   ...
   ``taktora-idl-wire`` primitives (:need:`REQ_0935`) and never ``serde``
   (:need:`REQ_0934`). Naming and field classification arrive already
   resolved from :need:`REQ_0939`; the backend adds only the wire format.
