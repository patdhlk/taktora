...
fn run_expand_prints_the_generated_module() {
    ...
    let source = ethercat_netcfg_cli::run_expand(&yaml_path).expect("run_expand succeeds");
    ...
    let config = ethercat_netcfg::parse(NETWORK_YAML).expect("fixture parses");
    let expected = ethercat_netcfg_codegen::generate(&config).expect("fixture generates");
    assert_eq!(
        source, expected,
