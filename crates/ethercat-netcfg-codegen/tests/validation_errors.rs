...
//! [`ethercat_netcfg_codegen::validate`] (and therefore `generate`) returns
//! the specific matching [`ValidationError`] variant. A valid baseline
//! config returns `Ok`.

use ethercat_netcfg_codegen::{ValidationError, validate};
...
fn validate_yaml(yaml: &str) -> Result<(), ethercat_netcfg_codegen::CodegenError> {
    let cfg = ethercat_netcfg::parse(yaml).expect("fixture parses");
    validate(&cfg)
}
