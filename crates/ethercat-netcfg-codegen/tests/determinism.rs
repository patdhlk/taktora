...
fn generated_source_is_byte_deterministic() {
    let cfg = ethercat_netcfg::parse(CANONICAL_YAML).expect("canonical config parses");

    // Sanity: the generated source is valid Rust.
    let first = ethercat_netcfg_codegen::generate(&cfg).expect("codegen succeeds");
    ...
    for iteration in 0..50 {
        let again = ethercat_netcfg_codegen::generate(&cfg).expect("codegen succeeds");
        assert_eq!(
            first, again,
    ...
    let reordered_cfg = ethercat_netcfg::parse(REORDERED_YAML).expect("reordered config parses");
    assert_eq!(
        cfg, reordered_cfg,
        "fixtures must be semantically identical; only key order may differ"
    );
    let reordered_out =
        ethercat_netcfg_codegen::generate(&reordered_cfg).expect("codegen succeeds");
    assert_eq!(
        first, reordered_out,
